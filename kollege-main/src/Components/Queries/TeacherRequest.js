...
const TeacherRequest = () => {
  ...
  // Assume the teacher is logged in with this name.
  ...
  const filteredComplaints = complaints.filter(complaint => {
    ...
    const matchesDomain = filterDomain === "All" || complaint.domain === filterDomain;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4">
      <header className="bg-gradient-to-r from-blue-500 to-indigo-600 py-6 shadow-lg mb-8">
        <h1 className="text-4xl font-bold text-center">Teacher Request Dashboard</h1>
      </header>
        ...
        <div className="grid gap-6">
          {filteredComplaints.map(complaint => (
            <div key={complaint._id} className="p-6 bg-gray-800 rounded-lg shadow-lg">
              ...
              <p className="text-sm text-gray-400 mb-2">Tracking ID: {complaint.trackingId}</p>
              {/* Teacher view always shows the actual student name */}
              <p className="text-sm text-gray-400">
                Student: {complaint.studentNameActual || "Anonymous"}
              </p>
